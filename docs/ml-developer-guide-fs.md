...
* Feature computation modules under `project1/feature_engineering` folder. 
...
* Python unit tests for feature computation modules in `project1/tests/feature_engineering` folder.
* Feature engineering notebook, `project1/feature_engineering/notebooks/GenerateAndWriteFeatures.py`, that reads input dataframes, dynamically loads feature computation modules, executes their `compute_features_fn` method and writes the outputs to a Feature Store table (creating it if missing).
...
* An automated integration test is provided (in `.github/workflows/project1-run-tests-fs.yml`) that executes a multi task run on Databricks involving the feature engineering and model training notebooks.
...
1. Implement your feature module, address TODOs in `project1/feature_engineering/features` and create unit test in `project1/tests/feature_engineering`
2. Update `project1/resources/feature-engineering-workflow-resource.yml`. Fill in notebook parameters for `write_feature_table_job`.
3. Update training data path in `project1/resources/model-workflow-resource.yml`.

We expect most of the development to take place in the `project1/feature_engineering` folder.

## Iterating on ML code

### Deploy ML code and resources to dev workspace using Bundles

Refer to [Local development and dev workspace](../project1/resources/README.md#local-development-and-dev-workspace)
...
You can iterate on ML code by running the provided `project1/feature_engineering/notebooks/GenerateAndWriteFeatures.py` notebook on Databricks using
...
#### Prerequisites
* Python 3.8+
* Install feature engineering code and test dependencies via `pip install -I -r project1/requirements.txt -r test-requirements.txt` from project root directory.
